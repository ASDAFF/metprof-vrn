;(function() {
	...
	BX.Landing.UI.Field.LinkURL.prototype = {
		...
		getBlockData: function(block)
		{
			...
			{
				var cacheResult = Cache.get(hash(block));
			...
			var resultPromise = BX.Landing.UI.Panel.URLList.getInstance()
				...
				.then(function(blocks) {
					var blockData = blocks.find(function(blockItem) {
						return blockId === blockItem.id;
					});

					var resultData = isPlainObject(blockData) ? {
						type: "block",
						id: blockData.id,
						name: blockData.name
					} : block;
			...
			Cache.set(hash(block), resultPromise);
