...
<div class="money-editor" id="<?=\Bitrix\Main\Text\HtmlFilter::encode($arParams['CONTROL_ID'])?>_wrap">
	...
	<input type="text" id="<?=\Bitrix\Main\Text\HtmlFilter::encode($arParams['CONTROL_ID'])?>_number" value="<?=\Bitrix\Main\Text\HtmlFilter::encode($arResult['VALUE_NUMBER'])?>" />&nbsp;<?if($arParams['EXTENDED_CURRENCY_SELECTOR'] === 'Y'):?><span id="<?=\Bitrix\Main\Text\HtmlFilter::encode($arParams['CONTROL_ID'])?>_currency_selector" class="money-editor-currency-selector-wrap"></span><?else:?><select <?if(strlen($arParams['FIELD_NAME_CURRENCY']) > 0):?>name="<?=\Bitrix\Main\Text\HtmlFilter::encode($arParams['FIELD_NAME_CURRENCY'])?>" <?endif;?>id="<?=\Bitrix\Main\Text\HtmlFilter::encode($arParams['CONTROL_ID'])?>_currency" onchange="BX.Currency.MoneyInput.get('<?=\Bitrix\Main\Text\HtmlFilter::encode(\CUtil::JSEscape($arParams['CONTROL_ID']))?>').setCurrency(this.value)">
<?
